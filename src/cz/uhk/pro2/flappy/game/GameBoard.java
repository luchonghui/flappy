...
public class GameBoard implements TickAware {
	...
	/**
	 * Vykresli celou herni plochu (sloupy, bonusy, ptak) na platno g. a
	 * kontroluje jestli pták narazil do nežádoucích dlaždic
	 * 
	 * @param g
	 */
				...
				if (t != null) {
					...
					if (t instanceof WallTile) {// je dlazdice typu zed?
						if (bird.collidesWithRectangle(screenX, screenY, Tile.SIZE, Tile.SIZE)) {
							// gameover = true; //doslo ke kolizi, hra ma
							// skoncit
						}
					}
					...
					if (t instanceof BonusTile) {
						if (bird.collidesWithRectangle(screenX, screenY, Tile.SIZE, Tile.SIZE)) {
							((BonusTile) t).sezrano(true); // ptak snedl bonus
						}
					}

					if (t instanceof BonusTile && j == (maxJ - 2)) {//
						((BonusTile) t).sezrano(false);
					}
				}
			}
		}
